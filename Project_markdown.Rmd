---
title: '[Unit 1 Project Template/Name your project here]'
authors: '[Team Number and Names]'
date: 
output: html_notebook

---
## Delete lines 8 - 16 section before submitting the file
This is an [R Markdown](http://rmarkdown.rstudio.com) Notebook to use as a template for your Unit Project 1.

Add new chunks by clicking the *Insert Chunk* button on the toolbar or by pressing *Ctrl+Alt+I*.

When you save the notebook, an HTML file containing the code and output will be saved alongside it. YOU DO NOT NEED TO KNIT. Instead, click Preview. The HTML file that is rendered will automatically be saved.

You will submit both the saved R Markdown file (.RMD) WITH A NEW NAME and the HTML file.
## Delete lines 8-16 before submitting the file.

**To:** [Add Name here and remove brackets]

**From:**Samson Joseph, Sean Simmons, Daniel Gurwah

**Subject:** [Add Subject here and remove brackets]


## Background
[Remove this text and replace with:  Provide a brief (one paragraph, please!) background description of 1) your task and 2) the context of your analysis. The context of your analysis should include: origin of dataset (if applicable), population of interest, data collection method, size of the data set, and a table of variables/factors to be explored with an explanation of what they mean and what type of variables they are.]

## Loading Dataset
```{r}
vg_sales <- read.csv('video_games_sales_1985_2016.csv')
vg_sales[,6:9]<-as.numeric(vg_sales$NA_Sales,vg_sales$EU_Sales, vg_sales$JP_Sales, vg_sales$Other_Sales)
vg_sales$Platform <- as.factor(vg_sales$Platform)
vg_sales$Publisher <- as.factor(vg_sales$Genre)
vg_sales$Genre <- as.factor(vg_sales$Genre)
vg_sales$Developer <- as.factor(vg_sales$Developer)
vg_sales$Rating <- as.factor(vg_sales$Rating)
vg_sales$Year_of_Release <- as.factor(vg_sales$Year_of_Release)
vg_sales$User_Score <- as.numeric(vg_sales$User_Score)
vg_sales$Name <- as.character(vg_sales$Name)
```

##Cleaning Data
```{r}
vg_sales$Rating[vg_sales$Rating %in% c("")]<- NA
vg_sales$Rating <-factor(vg_sales$Rating)
vg_sales$Developer[vg_sales$Developer %in% c("")]<- NA
vg_sales$Developer <-factor(vg_sales$Developer)

```
# Import data with all "N/A" and "tbd" replaced with "NA"
vgs <- read.csv('vg_sales with missing data replaced.csv')
vgs[,6:9]<-as.numeric(vgs$NA_Sales,vgs$EU_Sales, vgs$JP_Sales, vgs$Other_Sales)
vgs$Platform <- as.factor(vgs$Platform)
vgs$Publisher <- as.factor(vgs$Genre)
vgs$Genre <- as.factor(vgs$Genre)
vgs$Developer <- as.factor(vgs$Developer)
vgs$Rating <- as.factor(vgs$Rating)
vgs$Year_of_Release <- as.factor(vgs$Year_of_Release)
vgs$User_Score <- as.numeric(vgs$User_Score)
vgs$Name <- as.character(vgs$Name)
## Data Analyses

### Analysis 1: One Quantitative Variable
#### Analyst:Sean Simmons

[Put text of analysis here and add as many code chunks and text sections as you need.]

```{r}

```



### Analysis 2: One Categorical Variable
#### Analyst:[Name of Team Member]

[Put text of analysis here and add as many code chunks and text sections as you need.]


```{r}

```




### Analysis 3: Two Categorical Variables Joey
#### Analyst:[Name of Team Member]

[Put text of analysis here and add as many code chunks and text sections as you need.]

```{r}

```




### Analysis 4: One Categorical and One Quantitative Variable
#### Analyst:[Name of Team Member]

[Put text of analysis here and add as many code chunks and text sections as you need.]

```{r}

```




### Analysis 5: Two Quantitative Variable
#### Analyst:[Name of Team Member]

[Put text of analysis here and add as many code chunks and text sections as you need.]

```{r}

```





## Recommendations

[Add text section with your recommendations here. In this section, you will briefly summarize the most interesting features of your data. Are you seeing any trends, associations, or interesting features? Highlight these and make a recommendation that they be subjected to further statistical analysis. If there isn’t anything interesting, say so!]
